...
interface LoginPageProps {
  ...
  onOpenPhpPackage?: () => void;
}

// ============================================================================
// PENGATURAN TAMPILAN PINTASAN DEMO AKUN:
// Ubah konstanta SHOW_DEMO_SHORTCUTS di bawah menjadi false untuk
// menyembunyikan/menghilangkan panel tombol pintas demo akun secara permanen.
// ============================================================================
const SHOW_DEMO_SHORTCUTS = false;
...
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === 'admin') {
      // Check Admin credentials against adminUsers or default podtie13
      ...
      // Fallback check for podtie13 / Zionathan123
      ...
      setErrorMessage('Username Admin tidak terdaftar atau kata sandi salah.');
      return;
    }
  ...
  // Quick 1-click login handler for testing convenience
  const handleQuickLogin = (role: AuthRole, user: string, pass: string) => {
    setActiveTab(role);
    setUsername(user);
    setPassword(pass);
    setErrorMessage(null);
    ...
    if (role === 'admin') {
      const mainAdmin = adminUsers.find((a) => a.username.toLowerCase() === user.toLowerCase()) || {
        username: 'podtie13',
        name: 'Admin Utama (podtie13)',
      };
      onLogin({
      ...
      const t = teachers.find((tch) => tch.username?.toLowerCase() === user.toLowerCase());
      if (t) {
        onLogin({
          role: 'guru',
          teacherId: t.id,
  ...
  // Find sample homeroom teacher vs non-homeroom teacher for demo buttons
  const sampleHomeroom = teachers.find((t) => classes.some((c) => c.homeroomTeacherId === t.id)) || teachers[0];
  const sampleHomeroomClass = classes.find((c) => c.homeroomTeacherId === sampleHomeroom?.id);
  const sampleRegularTeacher = teachers.find((t) => !classes.some((c) => c.homeroomTeacherId === t.id)) || teachers[teachers.length - 1];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-100 via-slate-50 to-blue-50/40 flex flex-col justify-center items-center py-10 px-4 sm:px-6">
      <div className="w-full max-w-md">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={activeTab === 'guru' ? 'Masukkan username guru atau NIP...' : 'Masukkan username admin...'}
        ...
        {/* ====================================================================
            BAGIAN PINTAS MASUK AKUN DEMO (1-KLIK)
            Jika Anda ingin menghilangkan bagian ini, Anda dapat:
            1. Mengubah nilai konstanta di atas menjadi: const SHOW_DEMO_SHORTCUTS = false;
            ATAU
            2. Menghapus seluruh blok {SHOW_DEMO_SHORTCUTS && (...)} di bawah ini.
        ==================================================================== */}
        {SHOW_DEMO_SHORTCUTS && (
          <div className="mt-5 bg-white/90 backdrop-blur-xs rounded-xl p-4 border border-slate-200/80 shadow-xs text-xs">
            <div className="flex items-center gap-1.5 text-slate-700 font-bold mb-2">
              <Sparkles className="w-3.5 h-3.5 text-amber-500" />
              <span>Pintas Masuk Akun Demo (1-Klik Langsung Masuk)</span>
            </div>
            <p className="text-[11px] text-slate-500 mb-3">
              Pilih salah satu peran di bawah untuk menguji alur sistem secara langsung:
            </p>

            <div className="space-y-2">
              {/* 1. Guru yang sekalian Wali Kelas */}
              {sampleHomeroom && (
                <button
                  type="button"
                  id="btn-demo-guru-walikelas"
                  onClick={() =>
                    handleQuickLogin('guru', sampleHomeroom.username || 'budi', sampleHomeroom.password || 'guru123')
                  }
                  className="w-full text-left p-2.5 rounded-lg border border-blue-200 bg-blue-50/70 hover:bg-blue-100/80 transition-colors flex items-center justify-between group"
                >
                  <div>
                    <div className="font-bold text-[#1E3A6C] flex items-center gap-1.5">
                      <GraduationCap className="w-3.5 h-3.5 text-[#D8232A]" />
                      <span>{sampleHomeroom.name}</span>
                      <span className="text-[10px] bg-emerald-100 text-emerald-800 font-extrabold px-1.5 py-0.2 rounded border border-emerald-300">
                        Wali Kelas {sampleHomeroomClass?.name || 'X-A'}
                      </span>
                    </div>
                    <div className="text-[11px] text-slate-600 mt-0.5">
                      Bisa input nilai mapel + cetak rapor 1 kelas & per anak
                    </div>
                  </div>
                  <span className="text-[11px] font-bold text-[#1E3A6C] group-hover:underline shrink-0">
                    Masuk →
                  </span>
                </button>
              )}

              {/* 2. Guru Mapel saja (Bukan Wali Kelas) */}
              {sampleRegularTeacher && sampleRegularTeacher.id !== sampleHomeroom?.id && (
                <button
                  type="button"
                  id="btn-demo-guru-biasa"
                  onClick={() =>
                    handleQuickLogin(
                      'guru',
                      sampleRegularTeacher.username || 'andreas',
                      sampleRegularTeacher.password || 'guru123'
                    )
                  }
                  className="w-full text-left p-2.5 rounded-lg border border-slate-200 bg-slate-50 hover:bg-slate-100 transition-colors flex items-center justify-between group"
                >
                  <div>
                    <div className="font-bold text-slate-800 flex items-center gap-1.5">
                      <User className="w-3.5 h-3.5 text-slate-500" />
                      <span>{sampleRegularTeacher.name}</span>
                      <span className="text-[10px] bg-slate-200 text-slate-700 font-semibold px-1.5 py-0.2 rounded">
                        Guru Mapel Saja
                      </span>
                    </div>
                    <div className="text-[11px] text-slate-600 mt-0.5">
                      Hanya input nilai mapel yang dia ampu
                    </div>
                  </div>
                  <span className="text-[11px] font-bold text-[#1E3A6C] group-hover:underline shrink-0">
                    Masuk →
              ...
              <button
                type="button"
                id="btn-demo-admin"
                onClick={() => handleQuickLogin('admin', 'podtie13', 'Zionathan123')}
                className="w-full text-left p-2.5 rounded-lg border border-purple-200 bg-purple-50/70 hover:bg-purple-100/80 transition-colors flex items-center justify-between group"
              >
                <div>
                  <div className="font-bold text-purple-900 flex items-center gap-1.5">
                    <ShieldCheck className="w-3.5 h-3.5 text-purple-700" />
                    <span>Admin Utama (<span className="font-mono">podtie13</span>)</span>
                    <span className="text-[10px] bg-purple-200 text-purple-800 font-semibold px-1.5 py-0.2 rounded">
                      Super Admin
                    </span>
                  </div>
                  <div className="text-[11px] text-slate-600 mt-0.5">
                    ID: <span className="font-mono font-bold text-purple-900">podtie13</span> • Sandi: <span className="font-mono font-bold text-purple-900">Zionathan123</span>
                  </div>
                </div>
                <span className="text-[11px] font-bold text-purple-800 group-hover:underline shrink-0">
                  Masuk →
                </span>
              </button>
            </div>

            {/* PHP XAMPP Package Quick Download */}
            {onOpenPhpPackage && (
              <div className="mt-3 pt-3 border-t border-slate-200">
                <button
                  type="button"
                  onClick={onOpenPhpPackage}
                  className="w-full py-2.5 px-3 bg-emerald-50 hover:bg-emerald-100 text-emerald-900 border border-emerald-300 rounded-xl text-xs font-bold flex items-center justify-between transition-colors group"
                >
                  <div className="flex items-center gap-2">
                    <FolderArchive className="w-4 h-4 text-emerald-600" />
                    <div className="text-left">
                      <div>Paket Web PHP + MySQL (XAMPP)</div>
                      <div className="text-[10px] text-emerald-700 font-normal">
                        1 file 1 halaman standar, tanpa Node.js / npm
                      </div>
                    </div>
                  </div>
                  <span className="text-[11px] font-extrabold text-emerald-700 group-hover:underline">
                    Unduh ZIP →
                  </span>
                </button>
              </div>
